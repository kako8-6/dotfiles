...
readonly BASE_PKGS="$GRUB dialog cryptsetup lvm2 mdadm void-docs-browse dbus zramen brightnessctl nano flatpak xorg-minimal mesa-dri mesa-vulkan-intel mesa-vaapi mesa-vdpau xdg-user-dirs xdg-utils xdg-desktop-portal glxinfo bash-completion htop neofetch wget preload ffmpeg ufw thermald base-devel xtools git chrony pulseaudio alsa-plugins-pulseaudio font-adobe-source-code-pro fonts-droid-ttf noto-fonts-emoji liberation-fonts-ttf dejavu-fonts-ttf noto-fonts-ttf cantarell-fonts"
