...
readonly GNOME_PKGS="$BASE_PKGS gdm gnome polkit-gnome gvfs-goa gvfs-mtp gparted firefox"
readonly KDE_PKGS="$BASE_PKGS sddm kde5 kde5-baseapps ark spectacle kdialog kwrite mpv partitionmanager firefox"
...
readonly I3WM_PKGS="$BASE_PKGS i3-gaps i3lock i3status i3wsr dunst gvfs-mtp scrot network-manager-applet blueman gnome-themes-standard gnome-keyring feh scrot pcmanfm xarchiver lxterminal xdg-desktop-portal-gtk udisks2 udiskie pavucontrol papirus-icon-theme redshift lxappearance mpv firefox"
...
if [ -z "$IMAGE" -o "$IMAGE" = xfce ]; then
	if [ ! -e $XFCE_IMG ]; then
		./mklive.sh -a $ARCH -o $XFCE_IMG -i lz4 -s xz -v linux5.15 -l es_HN.UTF-8 -k la-latin1 -p "$XFCE_PKGS"  ${REPO} "$@"
	fi
fi
...
if [ -z "$IMAGE" -o "$IMAGE" = i3wm ]; then
        if [ ! -e $I3WM_IMG ]; then
                ./mklive.sh -a $ARCH -o $I3WM_IMG -i lz4 -s xz -v linux5.15 -l es_HN.UTF-8 -k la-latin1 -p "$I3WM_PKGS"  ${REPO} "$@"
        fi
fi
